...
let
  ...
  mkArchive = {
    ...
    let
      ...
      installLines = concatStringsSep "\n" (map (binary:
        "install -m0755 ${escapeShellArg (toString entries.${binary})} \"$out/bin/${binary}\"") binaries);
      ...
      archive = pkgs.runCommand "${pname}-${version}-${system}-portable-archive" {
        ...
        let
          ...
          entries = lib.mapAttrs (binary: entry:
            let
              ...
              wrapper = pkgs.runCommand "${pname}-${binaryName}-portable-entrypoint" {
                inherit version;
  ...
  mkPortableReleaseBinaryPackage = {
    ...
    let
      ...
      expectedBinaries = requireBinaries {context = "portable release"; inherit binaries;};
