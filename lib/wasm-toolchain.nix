...
"harbor-rs.mkWasmToolchain: requires pkgs with rust-overlay applied (pkgs.rust-bin must exist)";
assert pkgs.lib.assertMsg (builtins.elem channel ["nightly" "stable"])
"harbor-rs.mkWasmToolchain: 'channel' must be \"nightly\" or \"stable\", got \"${channel}\""; let
  channelSet =
    if channel == "nightly"
